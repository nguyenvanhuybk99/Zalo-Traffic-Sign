...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--dataset', default='coco',
                             help='coco | kitti | coco_hp | pascal | human')
    self.parser.add_argument('--exp_id', default='human')
    ...
    self.parser.add_argument('--debug_huy_dir', default='/home/momoinu/Desktop/anonymous/zalo_traffic_sign/CenterNet/src/debug',
                             help='location of debug')
    self.parser.add_argument('--debug_huy', default=False,
                             help='debug by huy :V')
    ...
    self.parser.add_argument('--save_dir', type=str, default='../exp/ctdet/test',
  ...
  def parse(self, args=''):
    ...
    opt.save_dir = os.path.join(opt.save_dir, str(datetime.now()))
    os.makedirs(opt.save_dir, exist_ok=True)
    
    opt.result_crop_dir = os.path.join(opt.save_dir, 'result_crop')
    os.makedirs(opt.result_crop_dir, exist_ok=True)
    
    opt.results_image_dir = os.path.join(opt.save_dir, 'result')
    os.makedirs(opt.results_image_dir, exist_ok=True)
  ...
  def init(self, args=''):
    ...
    'human': {'default_resolution': [512, 512], 'num_classes': 1,
              'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
              'dataset': 'human'},

    'coco': {'default_resolution': [512, 512], 'num_classes': 80,
              'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
              'dataset': 'coco'},

    'vehicle': {'default_resolution': [512, 512], 'num_classes': 5,
              'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
              'dataset': 'vehicle'}
    }
    class Struct:
    ...
    opt = self.parse(args)
    dataset = Struct(dataset_info[opt.dataset])
